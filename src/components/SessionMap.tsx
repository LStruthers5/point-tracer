...
export function SessionMap({
  ...
  const segmentRoute = activeSeg
    ? points.slice(activeSeg.start_idx, activeSeg.end_idx + 1).map((p) => [p.lat, p.lon] as [number, number])
    : null;
  ...
  const animatedRoute = animating && segmentRoute ? segmentRoute.slice(0, animIdx) : null;
      ...
      <div className="absolute top-3 right-3 z-[1000] flex flex-col gap-2">
        {activeSeg && (
          <button
            ...
            disabled={animating}
            className={`glass-card rounded-xl px-3 py-2 text-xs font-medium text-foreground cursor-pointer transition-all hover:border-primary/30 ${
